...
export async function POST(request: Request) {
  ...
  const newUser = await User.create({ email, password: hashedPassword ,id_rol: defaultRole._id, is_active: true, used_tokens: 0 })

  // haz que la fecha de inicio sea la fecha actual y la fecha de fin un mes despues    
  await UserPlan.create({ id_user: newUser._id, id_plan: defaultPlan._id, start_date: new Date(), end_date: new Date(new Date().setMonth(new Date().getMonth() + 1)), active: true  })
